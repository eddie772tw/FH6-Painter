...
*   **文字車貼生成器 (Text Vinyl Generator)**：支援將任意文字轉換為遊戲內向量幾何形狀，輕鬆製作文字車貼。
*   **實時畫布區域重繪 (ROI Painting)**：允許用戶在畫布上直接框選特定區域 (矩形/橢圓) 進行重新擬合，自動略過不需修改的背景，實現局部精細修復與優化。
*   **歷史回溯與接續生成 (History Rewind & Resume)**：提供歷史紀錄回溯功能，並支援中斷後無縫接續生成。
*   **效能跑分控制台 (Performance Benchmark Console)**：內建完整的效能基準測試套件，輕鬆評估不同硬體配置與 JIT 引擎組合的最佳表現。
...
*   **Text Vinyl Generator**: Convert arbitrary text into in-game vector geometry shapes easily.
...
*   **History Rewind & Resume**: Provide history rewind capabilities and support seamless resume of generation after interruption.
*   **Performance Benchmark Console**: Built-in comprehensive benchmarking suite to evaluate optimal performance across different hardware and JIT engine combinations.
