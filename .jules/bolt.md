...
## 2024-07-25 - Numba JIT Inner Loop Algebraic Factorization and Hoisting
**Learning:** In hot occlusion scanning loops within Numba JIT kernels, alpha compositing expressions like `val + (1.0 - val) * a_f` can be algebraically factored to `val * (1.0 - a_f) + a_f`. Since `a_f` is constant per shape, the term `1.0 - a_f` can be calculated once outside the inner loop.
**Action:** When working on alpha blending or accumulation passes in Numba kernels, proactively factor formulas to hoist constant subtractions out of the inner loop (converting 3 floating-point operations per pixel down to 2), which speeds up execution on large shape counts.
