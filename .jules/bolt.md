...
## 2026-06-05 - Array Slicing over Per-Pixel Iteration
**Learning:** In Pure Python environment kernels (`pure_python_evaluator.py`), explicitly iterating over pixels using nested loops (`for x in range(x_start, x_end + 1)`) is extremely slow due to Python bytecode evaluation overhead inside the hot loop.
**Action:** Replace manual per-pixel iterations with fast NumPy array slicing where applicable (e.g. `canvas[y, x_start:x_end+1, 0] = canvas[y, x_start:x_end+1, 0] * one_minus_a + r_val * a_f`). Vectorized array operations execute in C and bypass interpreter loop overhead, drastically improving execution speed in non-JIT pure Python fallbacks.
