...
## 2026-06-05 - Inner Loop Canvas Shape Check and Loop Hoisting
**Learning:** Checking `if canvas.shape[2] == 4:` inside a tight rendering inner loop causes massive overhead, especially in Pure Python (from ~3.5s to ~2.1s by hoisting). In addition, using NumPy slicing (`canvas[y, x_start:x_end+1, 0]`) rather than manually iterating over the `x` dimension pixel by pixel brings the Pure Python evaluation execution time down drastically (from ~2.1s to ~0.04s).
**Action:** Extract conditional shape checks outside of tight loops, branching to separate loops or assignments. Always utilize NumPy vectorization and slicing in purely Python loops wherever bounds can be predetermined to avoid the heavy interpretation overhead.
