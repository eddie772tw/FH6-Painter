...
def test_packaging_includes_all_resource_directories():
    ...
    spec_path = os.path.join(root_dir, "server-sidecar-x86_64-pc-windows-msvc.spec")
    ...
    for rdir in actual_resource_dirs:
        # 檢查 build_release.bat 是否包含該目錄的 --add-data 設定
        # 格式：--add-data "rdir\*...;rdir..."
        # 這裡使用正則表達式，容許引號、斜線等差異
        has_in_bat = False
        
        # 檢查是否含有例如 --add-data "lang...;lang" 或是 --add-data "tools...;tools" 等格式
        # PyInstaller format: --add-data "source;dest"
        # 我們檢查是否有包含該目錄名稱的 add-data 規則
        bat_pattern = rf'--add-data\s+["\']?{re.escape(rdir)}[^\n]*;{re.escape(rdir)}["\']?'
        if re.search(bat_pattern, bat_content) or f"{rdir}\\" in bat_content or f"{rdir};" in bat_content:
            has_in_bat = True
            
        assert has_in_bat, (
            f"根目錄下發現新的資源資料夾 '{rdir}'，但似乎未在 `build_release.bat` 的 --add-data 中進行配置。\n"
            f"若該資料夾需要打包發行，請將其新增至 `build_release.bat` 中。\n"
            f"若不需要打包，請將其列入 `tests/test_packaging.py` 的 `IGNORED_ROOT_DIRS` 中。"
        )

        # 檢查 spec 檔案的 datas 變數是否包含該目錄設定
        # 格式：('rdir\\*', 'rdir') 或其他包含該目錄字串的設定
        has_in_spec = False
        spec_pattern = rf'[\'"]{re.escape(rdir)}[\s\\*\'"]*,\s*[\'"]{re.escape(rdir)}[\'"]'
        if re.search(spec_pattern, spec_content) or f"'{rdir}\\" in spec_content or f'"{rdir}\\' in spec_content:
            has_in_spec = True
            
        assert has_in_spec, (
            f"根目錄下發現新的資源資料夾 '{rdir}'，但似乎未在 `server-sidecar-x86_64-pc-windows-msvc.spec` 的 datas 中進行配置。\n"
