...
if not HAS_DEPENDENCIES:
    print("ERROR: High-performance Python shape generator requires 'pillow' and 'numpy'.", file=sys.stderr)
...
def rebuild_uncovered_map_from_shapes(evaluator, width, height, has_alpha, alpha_mask, bias, shapes_list):
...
def run_generator(image_path, output_path=None, profile_path=None, layers_limit=None, candidates_limit=None, steps_limit=None, progress_callback=None, opt_settings=None, engine_name=None, taichi_arch=None, taichi_device_id=None):
    ...
    if uncovered_enabled:
        uncovered_map = evaluator.init_uncovered_map(width, height, has_alpha, uncovered_bias)
    ...
    try:
        while (len(shapes_list) - 1 < layers) and (attempts < max_attempts):
            ...
            if freeze_update_needed or weight_update_needed:
                ...
                if weight_update_needed:
                    ...
                    if max_err > 0:
                        ...
                        dynamic_weights = (1.0 + normalized_err * 9.0).astype(np.float32)
                        if boundary_enabled and has_alpha:
            ...
            if is_normal_trigger or is_test_trigger:
                ...
                shapes_list = evaluator.run_redundancy_check(shapes_list, width, height, final_check=False)
                evaluator.rebuild_canvas(canvas, shapes_list, avg_r, avg_g, avg_b)
                if uncovered_enabled:
                    uncovered_map = rebuild_uncovered_map_from_shapes(evaluator, width, height, has_alpha, alpha_mask, uncovered_bias, shapes_list)
                current_layer = len(shapes_list) - 1
...
def main():
    ...
    parser.add_argument("--engine", "-e", default="NUMBA", choices=["NUMBA", "TAICHI", "PURE_PYTHON"], help="Computational engine plugin to use (NUMBA, TAICHI, PURE_PYTHON)")
