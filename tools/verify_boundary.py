...
def verify_boundary(json_path, w=600.0, h=600.0):
    ...
    for idx, s in enumerate(shapes):
        ...
        if s.get("type") == 32:
            ...
            tolerance = 1e-4
            if (min_x < -tolerance or max_x > w + tolerance or 
                min_y < -tolerance or max_y > h + tolerance):
                ok = False
                errors.append({
                    "index": idx,
                    "center": (x_c, y_c),
    ...
    if ok:
        print("VERIFICATION SUCCESSFUL! All ellipses are 100% inside the canvas boundary.")
        return True
    else:
        print(f"VERIFICATION FAILED! Found {len(errors)} shapes violating the boundary limits.")
        for err in errors:
            print(f"  Shape index {err['index']}:")
            print(f"    Center: {err['center']}, Radii: {err['rx_ry']}, Theta: {err['theta']}")
            print(f"    X Range: [{err['x_range'][0]:.4f}, {err['x_range'][1]:.4f}] (Canvas Width: {w})")
            print(f"    Y Range: [{err['y_range'][0]:.4f}, {err['y_range'][1]:.4f}] (Canvas Height: {h})")
        return False
