...
def evaluate_candidate_ti(
    ...
    if is_valid == 1:
        ...
        # ⚡ Bolt Optimization: Forward differencing
        # Expanding (y - y_c)^2 into discriminant eliminates dy*dy and mults inside loops
        init_dy = ti.cast(min_y, ti.f32) - y_c
        dy_step = ti.cast(sample_step, ti.f32)
        init_dy_sq = init_dy * init_dy
        dy_sq_step = dy_step * dy_step
        two_init_dy_step = 2.0 * init_dy * dy_step

        init_b_val = init_dy * b_coeff
        b_val_step = dy_step * b_coeff

        init_discriminant = a - init_dy_sq * inv_rx2_ry2
        init_disc_step_1 = -two_init_dy_step * inv_rx2_ry2
        disc_step_2 = -dy_sq_step * inv_rx2_ry2
        disc_step_1_step = -2.0 * dy_sq_step * inv_rx2_ry2
        ...
        if check_contour == 1 and use_freeze == 0:
            y = min_y
            b_val = init_b_val
            discriminant = init_discriminant
            disc_step_1 = init_disc_step_1
...
def update_uncovered_mask_gpu(
    ...
    init_dy = ti.cast(min_y, ti.f32) - y_c
    init_dy_sq = init_dy * init_dy
    two_init_dy = 2.0 * init_dy

    b_val = init_dy * b_coeff

    discriminant = a - init_dy_sq * inv_rx2_ry2
    disc_step_1 = -two_init_dy * inv_rx2_ry2
    disc_step_2 = -inv_rx2_ry2
    disc_step_1_step = -2.0 * inv_rx2_ry2
...
def draw_ellipse_gpu(
    ...
    # ⚡ Bolt Optimization: Hoist multiplication outside inner-loop to save per-pixel ops
    r_val_af = r * a_f
    g_val_af = g * a_f
    b_val_af = b * a_f
