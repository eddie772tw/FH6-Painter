...
class NumbaEvaluator(BaseEvaluator):
    ...
    def rebuild_canvas(self, canvas: np.ndarray, shapes_list: list, avg_r: float, avg_g: float, avg_b: float) -> None:
        ...
        numba_kernels.rebuild_canvas_jit(canvas, avg_r, avg_g, avg_b, avg_a, shapes_data, shapes_color)
    ...
    def run_redundancy_check(self, shapes_list: list, width: int, height: int, final_check: bool = False) -> list:
        ...
        visible_mask = numba_kernels.run_redundancy_check_jit(shapes_data, shapes_color, shapes_type, width, height)
    ...
    def update_uncovered_mask(self, uncovered_map: np.ndarray, x_c: float, y_c: float, r_x: float, r_y: float, theta_rad: float) -> None:
        if not HAS_NUMBA:
            raise RuntimeError("Numba is not installed or available.")
        numba_kernels.update_uncovered_mask(uncovered_map, x_c, y_c, r_x, r_y, theta_rad)
