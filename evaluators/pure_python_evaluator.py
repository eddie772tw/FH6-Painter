...
def draw_ellipse_py(canvas, x_c, y_c, r_x, r_y, theta, r, g, b, alpha):
    ...
    r_af = np.float32(r_val * a_f)
    g_af = np.float32(g_val * a_f)
    b_af = np.float32(b_val * a_f)
    ...
    if has_alpha:
        for y in range(min_y, max_y + 1):
            ...
            if discriminant >= 0.0:
                ...
                if x_start <= x_end:
                    canvas[y, x_start:x_end+1, 0] = canvas[y, x_start:x_end+1, 0] * one_minus_a + r_af
                    canvas[y, x_start:x_end+1, 1] = canvas[y, x_start:x_end+1, 1] * one_minus_a + g_af
                    canvas[y, x_start:x_end+1, 2] = canvas[y, x_start:x_end+1, 2] * one_minus_a + b_af
...
class PurePythonEvaluator(BaseEvaluator):
    ...
    def search_best_shape(
        ...
        if use_importance and error_prob.shape[0] > 1:
            ...
            x_c_arr = np.random.uniform(0.0, float(width), batch_size).astype(
                np.float32
            )
            y_c_arr = np.random.uniform(0.0, float(height), batch_size).astype(
                np.float32
            )
    ...
    def rebuild_canvas(
        ...
        for s in shapes_list:
            if s["type"] == 32:
                ...
                draw_ellipse_py(
                    canvas, x_c, y_c, r_x, r_y, math.radians(theta_deg), r, g, b, alpha
                )
